...
const GiftSection: React.FC = () => {
  ...
  return (
    <section className="text-center space-y-6 py-12 bg-[#f9f8f6] -mx-6 px-10 rounded-[40px] shadow-inner">
      ...
      <h3 className="uppercase tracking-[0.3em] font-bold text-xs text-gray-800">{t('gift.title')}</h3>
        ...
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl animate-in fade-in zoom-in duration-500 relative">
          <div className="space-y-4 text-sm text-gray-600 text-left">
            <div className="border-b border-gray-50 pb-2">
              <p className="font-bold text-gray-400 text-[9px] uppercase tracking-widest mb-1">{t('gift.bankTitle')}</p>
              <p className="font-medium text-gray-700">{bankData.banco}</p>
            </div>
            <div className="border-b border-gray-50 pb-2">
              <p className="font-bold text-gray-400 text-[9px] uppercase tracking-widest mb-1">{t('gift.accountTypeTitle')}</p>
              <p className="font-medium text-gray-700">{bankData.tipo}</p>
            </div>
            <div className="border-b border-gray-50 pb-2">
              <p className="font-bold text-gray-400 text-[9px] uppercase tracking-widest mb-1">{t('gift.accountNumberTitle')}</p>
              <p className="font-mono bg-gray-50 p-2 rounded text-xs select-all text-gray-800 tracking-tight">{bankData.numero}</p>
            </div>
            <div className="border-b border-gray-50 pb-2">
              <p className="font-bold text-gray-400 text-[9px] uppercase tracking-widest mb-1">{t('gift.rutTitle')}</p>
              <p className="font-medium italic text-[#8fa189]">{bankData.rut}</p>
            </div>
            <div>
              <p className="font-bold text-gray-400 text-[9px] uppercase tracking-widest mb-1">{t('gift.holderTitle')}</p>
